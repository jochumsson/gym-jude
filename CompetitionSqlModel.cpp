...
{
    QSqlQuery query;
    query.prepare("SELECT competition_name, competition_date, team_competition, closed FROM competition WHERE competition_name=:competition_name_bind_value");
    ...
    m_current_competition_info = {
        ...
        competition_info.field(2).value().toBool(),
...
bool CompetitionSqlModel::update_competition(const CompetitionInfo & competition_info, QString & error_str)
{
    ...
    QSqlQuery query;
    query.prepare("UPDATE competition set competition_name=:competition_name_bind_value, competition_date=:competition_date_bind_value, team_competition=:team_competition_bind_value, closed=:closed_bind_value WHERE competition_name=:old_competition_name_bind_value");
